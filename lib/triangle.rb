class Triangle
  ...
  def valid? 
    ...
    if (@triangle_side.none? {|side| side <= 0}) && (total_one_two > @triangle_side[2] && total_two_three > @triangle_side[1] && total_two_three > @triangle_side[0])
      return true 
    else 
      false 
    end
  ...
  def kind 
    if valid?
      if @triangle_side.uniq.length == 3
        return :scalene
      elsif @triangle_side.uniq.lenth == 2 
        return :isosceles
      else 
        return :equilateral
      end
